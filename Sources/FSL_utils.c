...
uint8 MPL3115_Init(LDD_TDeviceData *DeviceDataPtr, struct PressureSensor *pthisPressure)
{
	...
	LDD_I2C_TErrorMask  MPL3115_I2C2_Error;
	...
	I2C2_Buf[0] = MPL3115_CTRL_REG1;		// byte 0 is the destination register
	I2C2_Buf[1] = 0x00;						// byte 1 is the data to write
	mqxglobals.I2C2_Status &= ~I2C_SENT_FLAG;			// reset the I2C sent flag

	// transmit the bytes
	I2C_MasterSendBlock(DeviceDataPtr, I2C2_Buf, 2, LDD_I2C_SEND_STOP);
	...
	do
	{
		...
		I2C_GetError(DeviceDataPtr, &MPL3115_I2C2_Error);
	}
	while ((!MPL3115_I2C2_Error) && !(mqxglobals.I2C2_Status & I2C_SENT_FLAG));

	// return immediately with error condition if MPL3115 is not present
	if (MPL3115_I2C2_Error)
		return false;
...
void FXOS8700_Init(LDD_TDeviceData *DeviceDataPtr, struct AccelSensor *pthisAccel, struct MagSensor *pthisMag)
{
	...
	I2C2_Buf[0] = FXOS8700_M_CTRL_REG1;
	I2C2_Buf[1] = 0x1F;
	mqxglobals.I2C2_Status &= ~I2C_SENT_FLAG;
	I2C_MasterSendBlock(DeviceDataPtr, I2C2_Buf, 2, LDD_I2C_SEND_STOP);
	// wait until the I2C sent callback function sets the sent flag
	...
	I2C2_Buf[0] = FXOS8700_M_CTRL_REG2;
	I2C2_Buf[1] = 0x20;
	mqxglobals.I2C2_Status &= ~I2C_SENT_FLAG;
	I2C_MasterSendBlock(DeviceDataPtr, I2C2_Buf, 2, LDD_I2C_SEND_STOP);
	// wait until the I2C sent callback function sets the sent flag
	...
	I2C2_Buf[0] = FXOS8700_XYZ_DATA_CFG;
	I2C2_Buf[1] = 0x01;
	mqxglobals.I2C2_Status &= ~I2C_SENT_FLAG;
	I2C_MasterSendBlock(DeviceDataPtr, I2C2_Buf, 2, LDD_I2C_SEND_STOP);
	// wait until the I2C sent callback function sets the sent flag
...
void FXAS21000_Init(LDD_TDeviceData *DeviceDataPtr, struct GyroSensor *pthisGyro)
{
	...
	I2C2_Buf[0] = FXAS21000_CTRL_REG1;
	I2C2_Buf[1] = 0x00;
	mqxglobals.I2C2_Status &= ~I2C_SENT_FLAG;
	I2C_MasterSendBlock(DeviceDataPtr, I2C2_Buf, 2, LDD_I2C_SEND_STOP);
	// wait until the I2C sent callback function sets the sent flag
	...
	I2C2_Buf[0] = FXAS21000_CTRL_REG0;
	I2C2_Buf[1] = 0x00;
	mqxglobals.I2C2_Status &= ~I2C_SENT_FLAG;
	I2C_MasterSendBlock(DeviceDataPtr, I2C2_Buf, 2, LDD_I2C_SEND_STOP);
	// wait until the I2C sent callback function sets the sent flag
...
void FXAS21002_Init(LDD_TDeviceData *DeviceDataPtr, struct GyroSensor *pthisGyro)
{
	...
	// [1-0]: Active=0, Ready=0 for Standby mode
	I2C2_Buf[0] = FXAS21002_CTRL_REG1;
	I2C2_Buf[1] = 0x00;
	mqxglobals.I2C2_Status &= ~I2C_SENT_FLAG;
	I2C_MasterSendBlock(DeviceDataPtr, I2C2_Buf, 2, LDD_I2C_SEND_STOP);
	// wait until the I2C sent callback function sets the sent flag
	...
	I2C2_Buf[0] = FXAS21002_CTRL_REG0;
	I2C2_Buf[1] = 0x00;
	mqxglobals.I2C2_Status &= ~I2C_SENT_FLAG;
	I2C_MasterSendBlock(DeviceDataPtr, I2C2_Buf, 2, LDD_I2C_SEND_STOP);
	// wait until the I2C sent callback function sets the sent flag
	...
	// place the gain in the gyro structure
#define FXAS21002_COUNTSPERDEGPERSEC 20.48F     			// for 1600dps test parts (1600dps=32768)
#define FXAS21002_DEGPERSECPERCOUNT 0.048828F				// must be reciprocal of FCOUNTSPERDEGPERSEC
	//#define FXAS21002_COUNTSPERDEGPERSEC 20.0F     			// for 1600dps test parts (1600dps=32000)
	//#define FXAS21002_DEGPERSECPERCOUNT 0.05F				// must be reciprocal of FCOUNTSPERDEGPERSEC
...
void MMA8652_Init(LDD_TDeviceData *DeviceDataPtr, struct AccelSensor *pthisAccel)
{
	...
	I2C2_Buf[0] = MMA8652_XYZ_DATA_CFG;
	I2C2_Buf[1] = 0x01;
	mqxglobals.I2C2_Status &= ~I2C_SENT_FLAG;
	I2C_MasterSendBlock(DeviceDataPtr, I2C2_Buf, 2, LDD_I2C_SEND_STOP);
	// wait until the I2C sent callback function sets the sent flag
...
void MPL3115_ReadData(LDD_TDeviceData *DeviceDataPtr, struct PressureSensor *pthisPressure)
{
	...
	I2C2_Buf[0] = MPL3115_OUT_P_MSB;
	mqxglobals.I2C2_Status &= ~I2C_SENT_FLAG;
	I2C_MasterSendBlock(DeviceDataPtr, I2C2_Buf, 1, LDD_I2C_NO_SEND_STOP);
	// wait until the I2C sent callback function sets the sent flag
	while ((mqxglobals.I2C2_Status & I2C_SENT_FLAG) == 0);

	// read the 5 bytes of sequential sensor data
	mqxglobals.I2C2_Status &= ~I2C_RCVD_FLAG;
	I2C_MasterReceiveBlock(DeviceDataPtr, I2C2_Buf, 5, LDD_I2C_SEND_STOP);
	// wait until the I2C received callback function sets the received flag
	while ((mqxglobals.I2C2_Status & I2C_RCVD_FLAG) == 0);
	// wait until the I2C bus is idle
	do
	{
		I2C_CheckBus(DeviceDataPtr, &BusState);
	...
	pthisPressure->iHpFast = (I2C2_Buf[0] << 24) | (I2C2_Buf[1] << 16) | (I2C2_Buf[2] << 8);
	pthisPressure->iTpFast = (I2C2_Buf[3] << 8) | I2C2_Buf[4];

	// use this line if the MPL3115 is to be used in pressure mode
	//pthisPressure->iPpFast = (I2C2_Buf[0] << 16) | (I2C2_Buf[1] << 8) | (I2C2_Buf[2] << 0);
...
void FXOS8700_ReadData(LDD_TDeviceData *DeviceDataPtr, struct AccelSensor *pthisAccel, struct MagSensor *pthisMag)
{
	...
	mqxglobals.I2C2_Status &= ~I2C_RCVD_FLAG;
	I2C_MasterReceiveBlock(DeviceDataPtr, I2C2_Buf, 12, LDD_I2C_SEND_STOP);
	// wait until the I2C received callback function sets the received flag
	while ((mqxglobals.I2C2_Status & I2C_RCVD_FLAG) == 0);
	// wait until the I2C bus is idle
	do
	...
	pthisAccel->iGpFast[X] = (I2C2_Buf[0] << 8) | I2C2_Buf[1];
	pthisAccel->iGpFast[Y] = (I2C2_Buf[2] << 8) | I2C2_Buf[3];
	pthisAccel->iGpFast[Z] = (I2C2_Buf[4] << 8) | I2C2_Buf[5];
	pthisMag->iBpFast[X] = (I2C2_Buf[6] << 8) | I2C2_Buf[7];
	pthisMag->iBpFast[Y] = (I2C2_Buf[8] << 8) | I2C2_Buf[9];
	pthisMag->iBpFast[Z] = (I2C2_Buf[10] << 8) | I2C2_Buf[11];
...
void FXAS21000_ReadData(LDD_TDeviceData *DeviceDataPtr, struct GyroSensor *pthisGyro, int16 irow)
{
	...
	mqxglobals.I2C2_Status &= ~I2C_RCVD_FLAG;
	I2C_MasterReceiveBlock(DeviceDataPtr, I2C2_Buf, 6, LDD_I2C_SEND_STOP);
	// wait until the I2C received callback function sets the received flag
	while ((mqxglobals.I2C2_Status & I2C_RCVD_FLAG) == 0);
	// wait until the I2C bus is idle
	do
	...
	pthisGyro->iYpFast[irow][X] = (I2C2_Buf[0] << 8) | I2C2_Buf[1];
	pthisGyro->iYpFast[irow][Y] = (I2C2_Buf[2] << 8) | I2C2_Buf[3];
	pthisGyro->iYpFast[irow][Z] = (I2C2_Buf[4] << 8) | I2C2_Buf[5];
...
void FXAS21002_ReadData(LDD_TDeviceData *DeviceDataPtr, struct GyroSensor *pthisGyro, int16 irow)
{
	...
	// debug: negate two channels on initial incorrectly trimmed samples
	pthisGyro->iYpFast[irow][X] = -pthisGyro->iYpFast[irow][X];	
	pthisGyro->iYpFast[irow][Z] = -pthisGyro->iYpFast[irow][Z];
...
void MAG3110_ReadData(LDD_TDeviceData *DeviceDataPtr, struct MagSensor *pthisMag)
{
	...
	pthisMag->iBpFast[X] = (I2C2_Buf[0] << 8) | I2C2_Buf[1];
	pthisMag->iBpFast[Y] = (I2C2_Buf[2] << 8) | I2C2_Buf[3];
	pthisMag->iBpFast[Z] = (I2C2_Buf[4] << 8) | I2C2_Buf[5];
...
// debug ifdef to prevent compiler warnings until we increase the size of the input buffer
#if defined SIMULATEDBT
// MATLAB: read simulated accelerometer and magnetometer data over Bluetooth
void ReadBluetoothAccelData(struct AccelSensor *pthisAccel)
{
	// place bytes 4 to 9 (ignoring first 4 bytes 0 to 3) into the 16 bit accelerometer structure
	// the most significant byte is transmitted before the least significant byte
	pthisAccel->iGpFast[X] = (sUARTInputBuf[4] << 8) | sUARTInputBuf[5];
	pthisAccel->iGpFast[Y] = (sUARTInputBuf[6] << 8) | sUARTInputBuf[7];
	pthisAccel->iGpFast[Z] = (sUARTInputBuf[8] << 8) | sUARTInputBuf[9];
...
{
	// place bytes 10 to 15 into the 16 bit magnetometer structure
	// the most significant byte is transmitted before the least significant byte
	pthisMag->iBpFast[X] = (sUARTInputBuf[10] << 8) | sUARTInputBuf[11];
	pthisMag->iBpFast[Y] = (sUARTInputBuf[12] << 8) | sUARTInputBuf[13];
	pthisMag->iBpFast[Z] = (sUARTInputBuf[14] << 8) | sUARTInputBuf[15];

	return;
}

// MATLAB: read simulated gyro data over Bluetooth
void ReadBluetoothGyroData(struct GyroSensor *pthisGyro, int16 irow)
{
	// place bytes 16 to 21 inclusive into the 16 bit gyro structure
	// the most significant byte is transmitted before the least significant byte
	pthisGyro->iYpFast[irow][X] = (sUARTInputBuf[16] << 8) | sUARTInputBuf[17];
	pthisGyro->iYpFast[irow][Y] = (sUARTInputBuf[18] << 8) | sUARTInputBuf[19];
	pthisGyro->iYpFast[irow][Z] = (sUARTInputBuf[20] << 8) | sUARTInputBuf[21];

	return;
}
#endif // end if Bluetooth sensor build
...
void CreateAndSendBluetoothPacketsViaUART(LDD_TDeviceData *DeviceDataPtr)
{
	...
	sBufAppendItem(sUARTOutputBuf, &iIndex, (uint8*)&(mqxglobals.FTM0_timestamp), 4);
	...
	// flags byte 33: least significant nibble: quaternion flags (changed Jan 2014)
	// Q3:   0xn1
	// Q3M:	 0xn6
	// Q3G:	 0xn3
	// Q6MA: 0xn2
	...
	// flags byte 33: most significant nibble (not changed Jan 2014)
	// NED:       0x0n     
	// Android:	  0x1n
	// Windows 8: 0x2n
	...
	case Q3:
		...
		iDelta = 0;
		isystick = (int16)(thisSV_3DOF_G_BASIC.systick / 20);
#endif
		break;
	case Q3M:
		...
		iDelta = 0;
		isystick = (int16)(thisSV_3DOF_B_BASIC.systick / 20);
#endif
		break;
	case Q3G:
		...
		iDelta = 0;
		isystick = (int16)(thisSV_3DOF_Y_BASIC.systick / 20);
#endif
		break;
	case Q6MA:
		...
		isystick = (int16)(thisSV_6DOF_GB_BASIC.systick / 20);
#endif
		break;
	case Q6AG:	
		...
		iDelta = 0;
		isystick = (int16)(thisSV_6DOF_GY_KALMAN.systick / 20);
#endif
		break;
	case Q9:
		...
		isystick = (int16)(thisSV_9DOF_GBY_KALMAN.systick / 20);
#endif
		break;
	case QCC:
#ifdef COMPUTE_6DOF_GY_CC
		fq = fqcc;
		flags |= 0x04;
		iPhi = (int16) (10.0F * PhVn);
		iThe = (int16) (10.0F * ThVn);
		iRho = 0;
		isystick = 0;
#endif
		break;
	...
	// correct the quaternion to ENU if NED
	...
	// ************************************************************************
	// Variable length debug packet type 2: Aug 2013 version
	...
	// ************************************************************************
	// Altitude / Temperature packet type 5: Jan 2014 version
	...
	// ************************************************************************
	// magnetic buffer packet type 6: Apr 2014 version
	...
	// ************************************************************************
	// Kalman filter packet type 7: Mar 2014 version
	...
	if ((globals.QuaternionPacketType == Q6AG) || (globals.QuaternionPacketType == Q9))
	{
		...
		// [4-3]: fThErrPl[X] resolution 0.0001 deg
		// [6-5]: fThErrPl[Y] resolution 0.0001 deg
		// [8-7]: fThErrPl[Z] resolution 0.0001 deg
		for (i = X; i <= Z; i++)
		{
			ftmp = 0.0F;
#if defined COMPUTE_6DOF_GY_KALMAN
			if (globals.QuaternionPacketType == Q6AG)
				ftmp = thisSV_6DOF_GY_KALMAN.fThErrPl[i] * 10000.0F;
#endif
#if defined COMPUTE_9DOF_GBY_KALMAN
			if (globals.QuaternionPacketType == Q9)
				ftmp = thisSV_9DOF_GBY_KALMAN.fThErrPl[i] * 10000.0F;
#endif
			// check for clipping
			if (ftmp > 32767.0F)
				tmpint16 = 32767;
		...
		// [16-15]: fbErrPl[X] resolution 0.001deg/s
		// [18-17]: fbErrPl[Y] resolution 0.001deg/s
		// [20-19]: fbErrPl[Z] resolution 0.001deg/s		
		for (i = X; i <= Z; i++)
		{
			ftmp = 0.0F;
#if defined COMPUTE_6DOF_GY_KALMAN
			if (globals.QuaternionPacketType == Q6AG)
				ftmp = (thisSV_6DOF_GY_KALMAN.fbErrPl[i] * 1000.0F);
#endif
#if defined COMPUTE_9DOF_GBY_KALMAN
			if (globals.QuaternionPacketType == Q9)
				ftmp = (thisSV_9DOF_GBY_KALMAN.fbErrPl[i] * 1000.0F);
#endif
			// check for clipping
			if (ftmp > 32767.0F)
				tmpint16 = 32767;
